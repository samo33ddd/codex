...
Activation is deferred if a Codex client is running, the local-control RPC cannot prove every thread is not loaded, or any status is unknown. A prepared candidate is tried first on the next run. Safe activation runs the candidate's `bin/codex.exe app-server daemon update --from-cli --yes` for each configured `CODEX_HOME`; status and a bounded log are written under `artifactsRoot`.
