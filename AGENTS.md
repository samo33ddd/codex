# Commit и push в этом проекте

- После завершения согласованной задачи и необходимых проверок самостоятельно делай commit и push в `origin` текущей рабочей ветки, без отдельного напоминания или повторного подтверждения. Это постоянное разрешение пользователя для этого репозитория, если в конкретной задаче не указано иначе.
- Включай в коммит только изменения текущей задачи; сохраняй чужие и незавершённые изменения. Перед push проверяй состав коммита, ветку и remote, после push — совпадение локального и удалённого SHA.
- Не используй force push и не расширяй это разрешение на merge, deploy или публикацию в другие репозитории. В итоговом ответе указывай коммит, ветку и результат push; при сбое сообщай конкретную причину.
